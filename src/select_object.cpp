...
actions::SelectObject::SelectObject(page_cache::PageCache &page_cache, std::string table_name, uint32_t row_id)
    : _page_cache(page_cache), _table_name(std::move(table_name)), _row_id(row_id) {
}
...
tuple::Tuple actions::SelectObject::get() {
    ...
    auto data_page = dynamic_cast<page::SlottedDataPage *>(
        _page_cache.get_page_id(record.page_id, _table_name, page::PageType::Data));
